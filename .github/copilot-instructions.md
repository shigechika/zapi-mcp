...
`mcp.run()` is reached — the `--version` / `--check` / `--brief` branches,
plus the missing-env-var check that runs ahead of all three — so none of
them ever run concurrently with the live stdio server. Flag any new code path
that adds a `print()`, or a logger without an explicit stderr handler, that
could execute while `mcp.run()` is active — that would be a new failure mode,
