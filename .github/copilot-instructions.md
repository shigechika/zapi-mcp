...
codebase has no `logging` usage at all, and every `print()` call lives in
`__main__.py`'s `--version` / `--check` / `--brief` branches — each of which
`sys.exit()`s (or returns) *before* `mcp.run()` is reached, so they never run
...
execute while `mcp.run()` is active — that would be a new failure mode, not
